...
policy = path.join(here, "policy")
assert path.exists(thirdparty_libs)
assert path.exists(policy)
...
#TODO ADD Policy file By BlackYe.
has_policy = policy in sys.path
if not (has_here and has_policy):
    if has_here:
        sys.path.remove(here)
    if has_thirdparty_libs:
        sys.path.remove(policy)
